...
const SystemLogsPage = () => {
  ...
  const fetchStats = async () => {
    try {
      const response = await fetch('http://localhost:5001/api/logs/stats');
      const result = await response.json();
  ...
  const exportLogs = async (format = 'json') => {
    try {
      const response = await fetch(`http://localhost:5001/api/logs/export?format=${format}`);
